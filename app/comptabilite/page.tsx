...
import { getFacturesListe, calculerTotaux, ApiError } from "@/lib/api";
...
export default function ComptabilitePage() {
  ...
  // Livre des recettes : uniquement les factures réellement encaissées,
  // sur l'année sélectionnée, triées chronologiquement par date d'encaissement.
  const recettes = useMemo(() => {
    return factures
      ...
      .map((f) => {
        ...
        return { facture: f, totalHt, totalTva, totalTtc };
      })
      .sort(
        (a, b) =>
          new Date(a.facture.payee_le!).getTime() -
          new Date(b.facture.payee_le!).getTime()
      );
  }, [factures, annee]);

  const totalAnnuel = recettes.reduce((s, r) => s + r.totalTtc, 0);
  const totalAnnuelHt = recettes.reduce((s, r) => s + r.totalHt, 0);
  ...
  const parMois = useMemo(() => {
    ...
    recettes.forEach((r) => {
      const m = new Date(r.facture.payee_le!).getMonth();
      tableau[m].nb += 1;
      tableau[m].totalHt += r.totalHt;
      tableau[m].totalTtc += r.totalTtc;
    });
    return tableau;
  }, [recettes]);

  const parTrimestre = useMemo(() => {
    const t = [0, 1, 2, 3].map((i) => ({ trimestre: i + 1, totalHt: 0, totalTtc: 0 }));
    parMois.forEach((m, i) => {
      const q = Math.floor(i / 3);
      t[q].totalHt += m.totalHt;
      t[q].totalTtc += m.totalTtc;
    });
    return t;
  }, [parMois]);

  // Impayés / en attente (toutes années) — vue complémentaire, hors livre des recettes
          ...
          <div className="print-doc space-y-8">
            ...
            <div>
              ...
              <h2 className="mt-1 font-display text-xl font-bold text-textPrimary">
                Livre des recettes — Année {annee}
              </h2>
            ...
            {/* KPIs */}
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 no-print">
              <div className="rounded-xl border border-line bg-surface p-4">
                <p className="text-[11px] uppercase tracking-wide text-textMuted">
                  Total encaissé {annee}
                </p>
                  ...
                  {parMois
                    ...
                    .map((m) => (
                      <tr key={m.mois} className="border-b border-line/50">
                        ...
                        <td className="py-2 text-right text-textMuted">{m.nb}</td>
                        <td className="py-2 text-right text-textMuted">
                          {m.totalHt.toFixed(2)} €
                        </td>
                        <td className="py-2 text-right font-medium text-textPrimary">
                          {m.totalTtc.toFixed(2)} €
                        </td>
                      </tr>
                    ))}
                  {recettes.length === 0 && (
            ...
            <div>
              <h3 className="mb-2 font-display text-sm text-textPrimary">
                Détail chronologique (livre des recettes)
              </h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-line text-left text-xs text-textMuted">
                  ...
                  {recettes.map((r) => (
                    <tr key={r.facture.id} className="border-b border-line/50">
                      <td className="py-2 text-textMuted">
                        {new Date(r.facture.payee_le!).toLocaleDateString("fr-FR")}
                      </td>
            ...
            <p className="text-[11px] text-textMuted no-print">
              Ce document liste uniquement les factures marquées "payée" (avec leur date
              d'encaissement réelle) — utile pour tes déclarations URSSAF et pour ton
              expert-comptable. Pense à marquer tes factures comme payées dès réception du
              règlement, sur la page Factures.
            </p>
          </div>
        )}
      </main>
